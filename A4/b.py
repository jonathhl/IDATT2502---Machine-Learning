...
index_to_char = [' ', 'h', 'r', 'c', 'f', 'm', 's', 'a', 'l', 'o', 't', 'p', 'n']
...
index_to_emojis = [emojis['hat'], emojis['rat'], emojis['cat'], emojis['flat'], emojis['matt'], emojis['cap'], emojis['son']]
emoji_encodings = np.eye(len(index_to_emojis))
emoji_size = len(emoji_encodings)


x_train = torch.tensor([[[char_encodings[1]], [char_encodings[7]], [char_encodings[10]], [char_encodings[0]]],  # "hat "
                        [[char_encodings[2]], [char_encodings[7]], [char_encodings[10]], [char_encodings[0]]],  # "rat "
                        [[char_encodings[3]], [char_encodings[7]], [char_encodings[10]], [char_encodings[0]]],  # "cat "
                        [[char_encodings[4]], [char_encodings[8]], [char_encodings[7]], [char_encodings[10]]],  # "flat"
                        [[char_encodings[5]], [char_encodings[7]], [char_encodings[10]], [char_encodings[10]]],  # "matt"
                        [[char_encodings[3]], [char_encodings[7]], [char_encodings[11]], [char_encodings[0]]],  # "cap "
                        [[char_encodings[6]], [char_encodings[9]], [char_encodings[12]], [char_encodings[0]]]],  # "son "
                       dtype=torch.float)
...
# Try to generate emoji fitting to string
def generate(string):
    model.reset()
    for i in range(len(string)):
        ...
        if i == len(string) - 1:
            print(index_to_emojis[y.argmax(1)], ": ", string)
...
#     if epoch % 10 == 9:
#         # Generate characters from the initial characters ' r'
#         model.reset()
#         text = ' r'
